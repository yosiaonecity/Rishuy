  ...
  it('passes', () => {
    ...
    cy.visit('https://uat.ladpc.net.il/ASAKIM-204/LoginUI?customerID=705870&autoUserID=040476400&autoUserPass=MA44ster&application=RISHUY_ASAKIM&userIP=127.0.0.1&ipFileNet')
      cy.url().should('include', 'https://uat.ladpc.net.il/ASAKIM-204/main');
        ...
        cy.wait(1000);
        cy.get('.search-lookup-street > .search-lookup > .mat-mdc-autocomplete-trigger').click().type('193');
        ...
        cy.get('.ng-star-inserted > .search-lookup > .mat-mdc-autocomplete-trigger').click().type('סביון');
        // cy.get('.search-lookup > .ng-dirty').type('{backspace}');
        // cy.get('bl-input-city-autocomplete.ng-untouched > .form-grid-cell > .search-lookup > .mat-mdc-autocomplete-trigger');
        // cy.get('.search-lookup > .ng-dirty').type('סביון');
        cy.get('.bl-list-row').click();
        // cy.get('.search-lookup > .ng-untouched').click();
        cy.get('.search-input > .search-lookup > .mat-mdc-autocomplete-trigger').type('193');
